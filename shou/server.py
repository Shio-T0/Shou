...
# the JSON IPC as a client-message — the playback watcher already reads that pipe, so it
# picks it up and hands the stream off to the phone.
...
CAST = {
    "active": False, "resolving": False, "url": "", "referer": "", "subtitle": "",
    "kind": "", "position": 0.0, "title": "", "episode": 0, "error": "", "gen": 0,
}
...
def anicli_play(gen: int, search_title: str, episode: int, display: str,
    ...
    player = f"mpv --fs --input-ipc-server={MPV_IPC} --input-conf={MPV_INPUT_CONF}"
...
def resolve_and_cast(gen: int, search: str, episode: int, title: str, position: float,
    ...
    we can reuse in the proxy) via the same scrapers used for playback."""
    with STATE_LOCK:
        if CAST["gen"] != gen:
            return
    ...
    broadcast()
    resolved = resolve_stream(search, episode)
    with STATE_LOCK:
        if CAST["gen"] != gen:
            return
        if not resolved:
...
def cast_stream():
    ...
    if is_m3u8:
        ...
        def rewrite(line: str) -> str:
            ...
            return _cast_proxy_path(urljoin(url, s))

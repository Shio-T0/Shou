#!/bin/bash
# Open AnimeUI: ask the (always-running) server to reset cleanly and show the kiosk.
...
if ! curl -s -o /dev/null -X POST "http://127.0.0.1:${PORT}/open"; then
  ...
  notify-send "🎌 AnimeUI" "Starting server…" -u low -t 1500
  nohup "$SCRIPT_DIR/animeui_daemon.sh" >/dev/null 2>&1 &
